...
# Cartopy-backed mapping helper used in place of mpl_toolkits.basemap.Basemap.
# Cartopy is optional until one of the map helpers is called.
try:
...
# Mapping helpers
# -----------------------------------------------------------------------------
# Basemap is deprecated/difficult to install in newer Python environments.  The
# small class below emulates only the Basemap behaviour used by this file:
#   1. m(lon, lat) -> projected x/y coordinates
#   2. drawmeridians(...)
#   3. drawparallels(...)
# This lets existing plotting code keep using ordinary Matplotlib axes rather
# than requiring every caller to create a Cartopy GeoAxes.  Cartopy is used only
# as the projection engine.


def _require_cartopy():
    if not _CARTOPY_AVAILABLE:
        raise ImportError(
            "Cartopy is required for make_map()/adjust_map(). Install it with "
            "conda install -c conda-forge cartopy, or add cartopy to your "
            "environment.yml."
        )


def _lcc_params(w_map, rotation):
    """Return the Lambert Conformal parameters used in the legacy Basemap code."""
    lon_0 = (w_map[1] - w_map[0]) / 2 + w_map[0] + rotation
    lat_0 = (w_map[3] - w_map[2]) / 2 + w_map[2]
    return lon_0, lat_0


class CartopyBasemap:
    """
    Minimal Basemap-compatible wrapper backed by Cartopy.

    This is intentionally small.  It is not a general replacement for Basemap,
    but it covers the calls used in this helper file and in downstream map code:

        m = CartopyBasemap(ax=ax, projection="lcc", ...)
        x, y = m(lon, lat)
        m.drawmeridians(...)
        m.drawparallels(...)
    """
    ...
    def __init__(self,
        ...
        if projection != 'lcc':
            raise NotImplementedError(
                "CartopyBasemap currently implements only the legacy 'lcc' "
                "projection used by this script."
            )

        if lon_0 is None or lat_0 is None:
            raise ValueError("lon_0 and lat_0 are required for the LCC projection.")
        ...
        self._last_meridians = None
        self._last_parallels = None

        if standard_parallels is None:
            standard_parallels = (lat_0, lat_0)

        self.src_crs = ccrs.PlateCarree()
        self.map_crs = ccrs.LambertConformal(
    ...
    def __call__(self, lon, lat):
        """Project longitude/latitude in degrees to map x/y coordinates."""
        lon_arr, lat_arr = np.broadcast_arrays(
            np.asarray(lon, dtype=float),
            np.asarray(lat, dtype=float)
        )
        scalar_input = lon_arr.ndim == 0
        lon_1d = np.atleast_1d(lon_arr)
        lat_1d = np.atleast_1d(lat_arr)

        pts = self.map_crs.transform_points(self.src_crs, lon_1d, lat_1d)
        x = pts[..., 0]
        y = pts[..., 1]

        if scalar_input:
            return float(x[0]), float(y[0])
        return x.reshape(lon_arr.shape), y.reshape(lat_arr.shape)
    ...
    def drawmeridians(self,
                      ...
                      zorder=1,
                      **kwargs):
        """Draw longitude gridlines on a normal Matplotlib axes."""
        meridians = np.asarray(list(meridians), dtype=float)
        self._last_meridians = meridians

        lat_min = self.llcrnrlat if self.llcrnrlat is not None else 47.0
        lat_max = self.urcrnrlat if self.urcrnrlat is not None else 51.0
        lats = np.linspace(lat_min, lat_max, 300)

        handles = {}
        for mer in meridians:
            lons = np.full_like(lats, mer)
            x, y = self(lons, lats)
            line, = self.ax.plot(
                x, y,
                linewidth=linewidth,
    ...
    def drawparallels(self,
                      ...
                      zorder=1,
                      **kwargs):
        """Draw latitude gridlines on a normal Matplotlib axes."""
        parallels = np.asarray(list(parallels), dtype=float)
        self._last_parallels = parallels

        if self._last_meridians is not None and len(self._last_meridians) > 1:
            lon_min = float(np.nanmin(self._last_meridians))
            lon_max = float(np.nanmax(self._last_meridians))
        else:
            # The old w_map lon bounds are narrow, so pad generously here.
            lon_min = self.llcrnrlon - 1.5 if self.llcrnrlon is not None else -125.5
            lon_max = self.urcrnrlon + 1.5 if self.urcrnrlon is not None else -122.0

        lons = np.linspace(lon_min, lon_max, 300)

        handles = {}
        for par in parallels:
            lats = np.full_like(lons, par)
            x, y = self(lons, lats)
            line, = self.ax.plot(
                x, y,
                linewidth=linewidth,
...
def create_lcc_map(ax,
                   w_map=[-124, -123.9, 47.7, 50.6],
                   rotation=39.2,
                   standard_parallels=None):
    """
    Create the Cartopy-backed Basemap shim used by make_map()/adjust_map().

    Returns
    -------
    CartopyBasemap
        Callable projection object.  Use `x, y = m(lon, lat)` as before.
    """
    lon_0, lat_0 = _lcc_params(w_map, rotation)
    return CartopyBasemap(
        ax=ax,
        projection='lcc',
        resolution='c',
        lon_0=lon_0,
...
def make_map(ax, grid, w_map=[-124, -123.9, 47.7, 50.6],
             ...
             bg_color='#e5e5e5'):
    """
    Draw the model bathymetry map using Cartopy instead of Basemap.

    This keeps the legacy API:
    ...
    ax : matplotlib.axes.Axes
        A normal Matplotlib axes is fine.  A Cartopy GeoAxes is not required.
    grid : xarray.Dataset-like
        Must contain nav_lon, nav_lat, and Bathymetry.
    w_map, rotation : list, float
        Legacy Lambert Conformal setup retained from the Basemap version.
    par_inc, mer_inc : float
        Spacing for latitude/longitude gridlines.
    fs : int
        Kept for backward compatibility; gridline labels are not drawn here.
    bg_color : str
        Fill colour for water/background bathymetry band.

    Returns
    -------
    CartopyBasemap
        Projection shim with Basemap-like call syntax.
    """
    fcolor = 'white'
    m = create_lcc_map(ax, w_map=w_map, rotation=rotation)

    lon = np.asarray(grid.nav_lon.values)
    lat = np.asarray(grid.nav_lat.values)
    bathy = np.asarray(grid.Bathymetry)
    x, y = m(lon, lat)

    ax.contourf(x, y, bathy, [-0.01, 0.01], colors=fcolor)
    ax.contour(x, y, bathy, [-0.01, 0.01], colors='black', linewidths=0.1)
    ax.contourf(x, y, bathy, [0.011, 500], colors=bg_color)

    m.drawmeridians(np.arange(-125.5, -122, mer_inc),
                    labels=[0, 0, 0, 0], linewidth=0.2, fontsize=fs)
    m.drawparallels(np.arange(47, 51, par_inc),
                    labels=[0, 0, 0, 0], linewidth=0.2, fontsize=fs)
...
def adjust_map(ax,
               ...
               label_grid=False):
    """
    Crop and label the map using projected coordinates.

    Pass the projection returned by make_map() as `m` to guarantee the same
    projection is used:

        m = make_map(ax, grid)
        adjust_map(ax, m=m, label_grid=True)

    If `m` is omitted, a default Strait-of-Georgia projection is created.
    """
    fcolor = 'white'

    if m is None:
        m = create_lcc_map(ax, w_map=[-124, -123.9, 47.7, 50.6], rotation=39.2)
    ...
    # Set height using map units.
    x_tl, y_tl = m(lon_tl, lat_tl)
    _, y_bl2 = m(lon_bl2, lat_bl2)
    ax.set_ylim(y_bl2, y_tl)

    # Cosmetic patch from the legacy Basemap version.
    ...
    if label_grid:
        fsmer = 7.5
        label_specs = [
            ("50.5N", -123.99, 50.43, -123.95, 50.43, -30),
            ("123.5W", -123.65, 50.05, -123.65, 50.15, 60),
            ("50N", -123.5, 49.94, -123.42, 49.94, -30),
            ("49.5N", -123.06, 49.4, -122.9, 49.4, -30),
            ("122.5W", -122.8, 49.15, -122.61, 49.15, 60),
            ("49N", -122.65, 49.0, -122.4, 48.93, -30),
        ]

        for label, lon_xy, lat_xy, lon_txt, lat_txt, rot in label_specs:
            ax.annotate(label,
                        xy=m(lon_xy, lat_xy),
                        xytext=m(lon_txt, lat_txt),
                        xycoords='data', textcoords='data',
                        ha='left', va='center',
                        fontsize=fsmer, rotation=rot)
...
#  2025-05-05 - added option to exclude_decjan
    ...
    if exclude_juntojan:
        df = df[~df['Date'].dt.month.isin([1, 5, 6, 7, 8, 9, 10, 11, 12])].copy()
    ...
    for year, group in grouped:
        ...
        for i in range(max(0, len(group) - 4)):  # Need i+4 below
